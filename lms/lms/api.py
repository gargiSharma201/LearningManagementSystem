...
def delete_member(user: str):
	...
	try:
		# Delete all linked LMS records first to avoid link validation errors
		for doctype in [
			"LMS Enrollment",
			"LMS Batch Member",
			"LMS Certificate",
			...
			linked = frappe.get_all(doctype, filters={"member": user} if doctype != "LMS Certificate" else {"member": user}, fields=["name"])
			for doc in linked:
		...
		frappe.log_error(frappe.get_traceback(), "delete_member failed")
		frappe.throw(_("Could not delete user: {0}").format(str(e)))
...
@frappe.whitelist()
def get_course_versions(course):
    return frappe.get_all(
        "Course Version",
...
@frappe.whitelist()
def restore_course_version(version_name):
    version = frappe.get_doc("Course Version", version_name)

    snapshot = json.loads(version.snapshot)

    course = frappe.get_doc("LMS Course", version.course)
    ...
    child_tables = ["chapters", "instructors"]

    # Restore normal fields
    for key, value in snapshot.items():
        if key in ignore_fields or key in child_tables:
            continue
...
@frappe.whitelist()
def get_course_version(version_name):
    version = frappe.get_doc("Course Version", version_name)
    return {
        "name": version.name,
...
@frappe.whitelist(allow_guest=True)
def enterprise_sign_up(email: str, full_name: str, password: str) -> dict:
	"""
	Custom signup for the Enterprise LMS.
	Creates a user with the password they entered and enables them immediately.
	No email verification step — suitable for a managed, internal LMS.
	"""
	...
	if frappe.db.exists("User", {"email": email}):
		existing = frappe.get_value("User", {"email": email}, ["enabled"], as_dict=True)
		if existing and existing.enabled:
			frappe.throw(_("An account with this email already exists. Please sign in."))
		else:
			frappe.throw(_("An account with this email exists but is disabled. Contact your administrator."))

	# Rate-limit: max 10 signups per hour from same IP
	try:
		users_past_hour = frappe.db.get_creation_count("User", 60)
		if users_past_hour >= 300:
			frappe.throw(_("Too many signups recently. Please try again later."))
	except Exception:
		pass

	# Create user with chosen password, enabled immediately
	user = frappe.get_doc({
		"doctype": "User",
		"email": email,
		...
		"send_welcome_email": 0,  # skip the random-password email
	})
	...
	# Assign default portal role if configured
	try:
